public class SurveyQuestions {

    /*
    Declare static final instance variables for interview questions below
    Would you like to join our survey? (Y/N) -> variable name can be askToJoin
    What is your name? -> variable name can be askName
    How old are you? -> variable name can be askAge
    What is your gender? (F/M) -> -> variable name can be askGender
     */

    public static final String askToJoin = "Would you like to join our survey? (Y/N)";

    public static final String askName = "What is your name?";

    public static final String askAge = "How old are you?";

    public static final String askGender = "What is your gender? (F/M)";
}